...
## ЗАДАЧА: напиши tagline для H1 первого экрана — это обещание конкретного результата.

ФОРМУЛА (выбери одну):
1. [Что клиент получит] + [за какое время] + [почему именно тут]
2. [Топ-2-3 услуги из Аспектов] + [город] + [конкретный результат]
3. [Главный страх] → [как мы его снимаем] + [город]

ЭТАЛОННЫЕ ПРИМЕРЫ:
- "Выходи красивой за 2 часа — макияж, ногти и брови в одном визите"
- "Идеальный маникюр + педикюр + укладка за один визит в Брянске"
- "Маникюр с гарантией 7 дней — переделаем бесплатно. Брянск."
- "Свадебный образ за одно посещение. Результат обсуждаем до, не после."

ВАЖНЫЕ ПРАВИЛА:
- НЕ начинай со слов «Студия», «Салон», «Салон красоты», «Цирюльня» и т.п.
- НЕ используй рейтинг и количество отзывов (они будут отдельным badge рядом с tagline)
- Используй топ-2-3 аспекта и/или популярные услуги из поля Аспекты
- Конкретика: что клиент получит, за какое время, почему именно тут
- НЕ используй штампы: «оазис», «команда профессионалов», «индивидуальный подход», «мир красоты»
...
Предложение 3: рейтинг как сухой факт БЕЗ числа отзывов — только «Рейтинг 5.0 из 5»
...
«это говорит о том что клиенты довольны!» → «Рейтинг 5.0 из 5»
«постараемся исправить» → «исправим бесплатно в течение 7 дней»

ВЕРХНЕЕ ПРАВИЛО: НЕ ИСПОЛЬЗУЙ число отзывов в тексте. Никогда. Даже если это кажется важным.
...
Верни JSON: {{\"services\":[{{\"title\":\"...\",\"short\":\"...\",\"description\":\"...\",\"priceFrom\":\"...\"}}]}}"""
    ...
    if isinstance(parsed, dict) and isinstance(parsed.get("services"), list):
        ...
        for item in parsed["services"]:
            ...
            # Ищем цену и описание в real_services по совпадению названия
            price = None
            original_description = None
            title_lower = title.lower()
            for real_name, real_data in real_prices_dict.items():
                if title_lower in real_name.lower() or real_name.lower() in title_lower:
                    if isinstance(real_data, dict):
                        price = compact_text(str(real_data.get("price", "")))
                        original_description = real_data.get("description", "")
                    else:
                        price = compact_text(str(real_data))
                    break
            ...
            # Если в original_description есть "от X до Y", извлекаем "от X" для цены
            # Перезаписываем цену LLM если найден паттерн в оригинальном описании
            if original_description:
                import re
                # Ищем паттерн "от Xр. до Yр." или "от X до Y" в original_description
                pattern = r'от\s*(\d+)\s*(?:р\.?|₽\.?)?\s*до\s*\d+'
                match = re.search(pattern, original_description)
                if match:
                    min_price = match.group(1)
                    extracted_price = f"от {min_price} ₽"
...
async def curate_lead(lead_id: int) -> Optional[Dict[str, Any]]:
    ...
    if card_data:
        ...
        for s in card_services:
            ...
            if name and price:
                card_prices[name.lower()] = {"price": price, "description": description}
